document.addEventListener('DOMContentLoaded', function() {
    ...
    // Attendre que toutes les ressources critiques soient chargées
    if (document.readyState === 'complete') {
        // Si déjà chargé, masquer immédiatement
        setTimeout(hideLoader, 300);
    } else {
        // Sinon, attendre le chargement complet avec un délai maximum
        ...
        // Délai maximum de sécurité (réduit de 2s à 1.5s)
        setTimeout(() => {
            if (!loadingComplete) {
                loadingComplete = true;
                hideLoader();
    ...
    // Navigation et scroll avec throttling pour les performances
    let scrollTimeout;
    window.addEventListener('scroll', function() {
        ...
        scrollTimeout = setTimeout(() => {
            ...
            animateSections();
        }, 10); // Throttle à 10ms pour de meilleures performances
    });
    ...
    // Animation de l'effet de typage pour le texte d'introduction - optimisée
    ...
    if (typeText) {
        ...
        // Lancer l'animation de typage après le chargement optimisé
        setTimeout(typeWriter, 800); // Réduit de 1500ms à 800ms
    }
